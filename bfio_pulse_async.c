													...
													void **buffers[2], int frame_count, int event))
{
	...
	while (PA_CONTEXT_CONNECTING == pa_context_get_state (my_pa_ctx))
	{
		fprintf (stderr, "Pulse I/O error connecting recording-stream, code %d.\n",
							pa_context_get_state (my_pa_ctx));
	};
	...
	if (io == BF_IN)
	{
		my_pa_stream_in = pa_stream_new_with_proplist (my_pa_ctx, pa_stream_name,
																										&pa_sample_spec,
																										pa_channel_map,
